...
public class HexGridChunk : MonoBehaviour
{
	...
	void TriangulateEstuary(EdgeVertices e1, EdgeVertices e2, bool incommingRiver, Vector3 indices)
    {
	...
	void TriangulateAdjacentToRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
		...
		if (cell.HasRiverThroughEdge(direction.Next()))
        {
            ...
            {
				center += HexMetrics.GetSolidMiddleEdge(direction) * (HexMetrics.InnerToOuter * 0.5f);
			}
	...
	void TriangulateRoadAdjacentToRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
		...
		if (cell.HasRiverBeginOrEnd)
        {
			roadCenter += HexMetrics.GetSolidMiddleEdge(cell.RiverBeginOrEndDirection.Opposite()) * (1f / 3f);
		}
		...
		else if (previousHasRiver && nextHasRiver)
        {
			...
			Vector3 offset = HexMetrics.GetSolidMiddleEdge(direction) * HexMetrics.InnerToOuter;
		...
		else
        {
            ...
            Vector3 offset = HexMetrics.GetSolidMiddleEdge(middle);
	...
	void TriangulateWithRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        {
			centerL = center;
			centerR = center + HexMetrics.GetSolidMiddleEdge(direction.Next()) * (0.5f * HexMetrics.InnerToOuter);
		}
		else
        {
			centerL = center + HexMetrics.GetSolidMiddleEdge(direction.Previous()) * (0.5f * HexMetrics.InnerToOuter);
            centerR = center;
		}
    ...
    void TriangulateEdgeFan(Vector3 center, EdgeVertices edge, float cellIndex)
    {
    ...
    void TriangulateEdgeStrip(EdgeVertices e1, Color w1, float index1, EdgeVertices e2, Color w2, float index2, bool hasRoad = false)
    {
        ...
        if (hasRoad)
        {
            TriangulateRoadSegment(e1.v2, e1.v3, e1.v4, e2.v2, e2.v3, e2.v4, weights1, weights2, indices);
        }
    }
